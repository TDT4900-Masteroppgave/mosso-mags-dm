...
for c_val in "${SAMPLE_SIZES[@]}"; do
    ...
    ./run.sh sweep --algorithm sm_thr --dataset PR EN FB DB YT --dynamic DB YT --param thr --range 0.1 1.1 0.1 --c "$c_val" &
    sleep 2

    ./run.sh sweep --algorithm top-b --dataset PR EN FB DB YT --dynamic DB YT --param b --range 1 10 1 --c "$c_val" &
    sleep 2

    ./run.sh sweep --algorithm cap --dataset PR EN FB DB YT --dynamic DB YT --param cap --range 10 120 10 --c "$c_val" &
    sleep 2

    ./run.sh sweep --algorithm ds_thr --dataset PR EN FB DB YT --dynamic DB YT --param T --range 1 100 10 --c "$c_val" &
