...
class AlgorithmRunner(ABC):
    ...
    def format_dataset(self, original_dataset_path: str) -> str:
        orig = Path(original_dataset_path)
        save_dir = orig.parent / self.__class__.__name__
        save_dir.mkdir(exist_ok=True)

        formatted_path = save_dir / f"{self.__class__.__name__}_{orig.name}"
        if not formatted_path.exists():
            self.logger.debug(f"\t[*] Cleaning dataset for {self.__class__.__name__}: {orig.name}")
            clean_and_write(str(orig), str(formatted_path), self.edge_format_string)
        return str(formatted_path)

    def run_multiple(self, dataset_path: str, base_output_name: str, n_runs: int, parameters: list) -> tuple[
        Optional[float], Optional[float], list, list]:
